...
\begin{enumerate}
  \item  La fonction \ci{split()}\index{split@\ci{split}}\index{chaine@chaîne!separer@séparer} est une méthode \Python{} qui sépare une chaîne de caractères en morceaux. Si aucun séparateur n'est préciser, le séparateur est le caractère espace.
  ...
  \item Partir avec une pile vide.   
   ...
   \begin{itemize}
     ...
     \item si le caractère est une parenthèse ou un crochet fermant  \ci{")"} ou \ci{"]"} :
       \begin{itemize}
         \item tester si la pile est vide, si elle vide alors renvoyer \og{}faux\fg{} (le programme se termine là, l'expression n'est pas cohérente), si la pile n'est pas vide continuer, 
