...
\emph{Remarque.} Dans tout ce cours, on écrira les \og{}nombres à virgule\fg{} sous la forme $3.5$ (et pas $3,5$). Le séparateur décimal est donc le point. En informatique les nombres à virgules sont appelés \og{}nombres flottants\fg{}\index{nombre flottant}.
\end{cours}
...
\begin{enumerate}
  ...
  \item $7$ est le premier entier tel que son inverse ait une écriture décimale périodique de longueur $6$ :
...
\begin{enumerate}
  \item  La fonction \Python{} est \ci{gcd(a,b)}\index{gcd@\ci{gcd}}\index{pgcd} (sans le \og{}p\fg{}, pour \emph{greatest common divisor}). Calcule le pgcd de $a = 10\,403$ et $b = 10\,506$. Déduis-en le ppcm\index{ppcm} de $a$ et $b$. La fonction ppcm n'existe pas, tu dois utiliser la formule :
