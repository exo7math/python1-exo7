...
\begin{itemize}
  ...
  \item Commande pour tracer un rectangle : \ci{create_rectangle(x1,y1,x2,y2)} ; il suffit de préciser les coordonnées $(x_1,y_1)$, $(x_2,y_2)$ de deux sommets opposés. L'option \ci{width} ajuste la taille du trait, \ci{outline} définit la couleur de ce trait, \ci{fill} définit la couleur de remplissage.
  ...
  \item Du texte est affiché par la commande \ci{canvas.create_text(x,y,text="Mon texte")} en précisant les coordonnées $(x,y)$ du point à partir duquel l'on souhaite afficher le texte. 
...
\end{itemize}
  ...
  \item \textbf{Graphique en pourcentage.} Écris une fonction \ci{graphique_pourcentage(liste)} qui affiche les valeurs d'une liste sous la forme d'un rectangle horizontal de taille fixe (par exemple $500$ pixels) et est divisé en sous-rectangles représentant les valeurs.
...
\begin{center}
\includegraphics[scale=0.9]{ecran-stat-4}
\end{center}
...
\begin{center}
\includegraphics[scale=0.8]{ecran-stat-5a}
\end{center}
...
\begin{center}
\includegraphics[scale=0.8]{ecran-stat-5b}
\end{center}
